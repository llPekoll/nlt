...
def update_cash_prices(week, cash_added):
    week.first_bounty += (cash_added*.5)
    week.second_bounty += (cash_added*.21)
    week.third_bounty += (cash_added*.09)
    return week
...
class WeekView(APIView):
    def get(self, request):
        ...
        week_dict['challenge_ticket'] = ChallengeTicket.objects.filter(week=week).count()
        week_dict['lives_used'] = LiveUsed.objects.filter(week=week).count()
        week_dict['nft_lives_used'] = LiveUsed.objects.filter(week=week,nft_used__gt=0).count()
