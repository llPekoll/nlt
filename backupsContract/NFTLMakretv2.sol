...
contract n2DMarket is ReentrancyGuard, Ownable {
  ...
  function getMarkertItem(uint256 _tokenId) public view returns (VaultItem memory) {
    return idToVaultItem[_tokenId];
  }

  function updateListPrice(uint256 _listPrice) public payable {
    require(holder == msg.sender, "Only owner can update listing price");
...
//   function createVaultItem(address nftContract,uint256 tokenId,uint256 price) public payable nonReentrant {
  function createVaultItem(address nftContract,uint256 tokenId) public payable nonReentrant {
    ...
    idToVaultItem[itemId] =  VaultItem(itemId,nftContract,tokenId,payable(msg.sender),payable(address(0)),false);
    IERC721(nftContract).transferFrom(msg.sender, address(this), tokenId);
    // emit VaultItemCreated(itemId,nftContract,tokenId,msg.sender,address(0),price,false);}
    emit VaultItemCreated(itemId,nftContract,tokenId,msg.sender,address(0),false);}
